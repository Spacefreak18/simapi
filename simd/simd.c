...
static void on_udp_recv(uv_udp_t *handle, ssize_t nread, const uv_buf_t *rcvbuf,
...
int startudp(int port) {
  ...
  int err = uv_udp_bind(&recv_socket, (const struct sockaddr *)&recv_addr,
                        UV_UDP_REUSEADDR);
...
void bridgeclosecallback(uv_timer_t *handle) {
  ...
  if (is_pid_running(f->game_pid) == 0) {
    y_log_message(Y_LOG_LEVEL_INFO,
                  "No longer detected game pid %i, so closing bridge pid %i",
                  f->game_pid, f->bridge_pid);
    ;

    if (simds.notify == true) {
      char cmd[512];
      snprintf(cmd, sizeof(cmd), "notify-send -t 3000 \"%s\" \"game stopped\"",
               "simd");
      system(cmd);
    }
...
void gamefindcallback(uv_timer_t *handle) {
  ...
  for (int i = 0; i < f->compat_info_size; i++) {
    ...
    if (gamepid > 0) {
      ...
      y_log_message(
          Y_LOG_LEVEL_INFO,
          "found a specified launch process for gameid %i running at pid %i.",
          sim, gamepid);
      break;
    }
  }
  ...
  if (sim > 0) {
    y_log_message(
        Y_LOG_LEVEL_INFO,
        "Detected simulator id %i, starting appropriate bridge if necessary.",
        sim);
    ...
    if (does_sim_need_bridge(sim) == true && err == 0 && i > -1) {
      ...
      pid_t pid = gamepid;
      const char *env_var1 =
          "STEAM_COMPAT_TOOL_PATHS"; // this will need to be interpreted
      const char *env_var2 =
          "STEAM_COMPAT_DATA_PATH"; // this will be exactly what we need except
                                    // append /pfx
      const char *env_var3 =
          "SIMD_BRIDGE_EXE"; // this will be exactly what we need
      const char *env_var4 =
          "SIMD_WRAP_EXE"; // this will be exactly what we need
      ...
      if (env_steam_compat_tool == NULL || env_steam_compat_data == NULL ||
          env_simd_bridge_exe == NULL) {
        err = -1;
        y_log_message(Y_LOG_LEVEL_WARNING,
                      "Could not find one or all of the necessary environment "
                      "variables. Found %s %s %s",
                      env_steam_compat_tool, env_steam_compat_data,
                      env_simd_bridge_exe);
        ...
        y_log_message(Y_LOG_LEVEL_DEBUG, "Retrieved env vars %s and %s and %s",
                      env_steam_compat_tool, env_steam_compat_data,
                      env_simd_bridge_exe);
      }
      ...
      if (err == 0) {
        ...
        if (wineexe != NULL) {
          y_log_message(Y_LOG_LEVEL_DEBUG, "Determined wine executable path %s",
                        wineexe);
        }
      }
      ...
      if (err == 0) {
        y_log_message(Y_LOG_LEVEL_DEBUG,
                      "No errors found, will attempt to fork a process like "
                      "this WINEFSYNC=1 %s %s %s",
                      wineprefix, wineexe, env_simd_bridge_exe);

        static char *newargv[] = {
            "/usr/bin/steam-run", "/usr/bin/wine",
            "/home/user/git/simshmbridge/assets/acbridge.exe", NULL};
        static char *newenviron[] = {"WINEPREFIX=/home/user/.local/share/Steam/"
                                     "steamapps/compatdata/244210",
                                     "WINEFSYNC=1", NULL};
        ...
        if (env_simd_wrap_exe == NULL) {
          ...
          y_log_message(Y_LOG_LEVEL_DEBUG, "Using wrap exe path %s",
                        env_simd_wrap_exe);
        ...
        if (process > 0) {
          y_log_message(Y_LOG_LEVEL_DEBUG,
                        "Fork was successful looking for data next");
      ...
      y_log_message(Y_LOG_LEVEL_DEBUG,
                    "sim %i does not require a compatibility exe, will "
                    "continue to mapping data",
                    sim);
    ...
    if (err == 0) {
      if (simds.notify == true) {
        ...
        snprintf(cmd, sizeof(cmd),
                 "notify-send -t 3000 \"%s\" \"Detected %s (%i)\"", "simd",
                 gamename, sim);
        system(cmd);
      }
    }
...
void datacheckcallback(uv_timer_t *handle) {
  ...
  if (f->simstate == true && simdata->simstatus >= 2) {
    if (appstate == 1) {
      ...
      uv_timer_stop(handle);
      // i can make this more frequent but i need to be conscious of resources,
      // don't want to trash anyone's frame rates
...
void cb(uv_poll_t *handle, int status, int events) {
  ...
  if (ch == 'q') {
    if (f->releasing == false) {
      appstate--;
      y_log_message(Y_LOG_LEVEL_INFO, "User requested stop appstate is now %i",
                    appstate);
      fflush(stdout);
    }
  }
...
int main(int argc, char **argv) {
  ...
  if (pid_file_fd == -1) {
    ...
    fprintf(stderr, "simd daemon already running, please remove /tmp/simd.pid "
  ...
  y_init_logs("simd", ylog_mode, Y_LOG_LEVEL_DEBUG, "/tmp/simd.log",
              "Initializing logs mode: file, logs level: debug");
  y_log_message(Y_LOG_LEVEL_INFO,
  ...
  char *configfile;
  asprintf(&simds.configfile, "%s%s", simds.home_dir,
           "/.config/simd/simd.config");
  config_t cfg;
  config_init(&cfg);
  if (!config_read_file(&cfg, simds.configfile)) {
    ...
    config_destroy(&cfg);
    y_log_message(Y_LOG_LEVEL_WARNING, "Error with config file: %s",
                  configerror);
    y_log_message(Y_LOG_LEVEL_WARNING, "Disabling Automatic Bridge Mode");
    free(configerror);
  } else {
    y_log_message(Y_LOG_LEVEL_INFO,
                  "Opened and validated simd configuration file");
    good_config = true;
  }
  config_destroy(&cfg);
  ...
  if (simds.daemon == false) {
    if (isatty(0)) {
      ...
      if (r == 0) {
        ...
        y_log_message(
            Y_LOG_LEVEL_WARNING,
            "Could not initialize stdin poll, interactive mode disabled");
      }
    } else {

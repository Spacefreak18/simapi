...
bool does_sim_need_bridge(SimulatorEXE s) {
  if (s == SIMULATOREXE_ASSETTO_CORSA ||
      s == SIMULATOREXE_ASSETTO_CORSA_COMPETIZIONE ||
      s == SIMULATOREXE_ASSETTO_CORSA_EVO ||
      s == SIMULATOREXE_ASSETTO_CORSA_RALLY) {
    return true;
  }
...
void SetProximityData(SimData *simdata, int cars, int8_t lr_flip) {
  ...
  for (int car = 1; car < cars; car++) {
    ...
    double radius = sqrt((rawXCoordinate * rawXCoordinate) +
                         (rawYCoordinate * rawYCoordinate));
    ...
    if (radius < maxradius) {
      ...
      if ((simdata->pd[j].radius == 0) || (radius < simdata->pd[j].radius)) {
        ...
        while (j >= 0) {
          ...
          if ((simdata->pd[j].radius == 0) ||
              (tempradius < simdata->pd[j].radius)) {
...
SimInfo getSim(SimData *simdata, SimMap *simmap, bool force_udp,
  ...
  if (simd == false) {
    if (does_sim_file_exist("/dev/shm/SIMAPI.DAT")) {
      ...
      free(temp);
      // simdatamap(simdata, simmap, NULL, SIMULATORAPI_SIMAPI_TEST, false,
      // NULL);
      if (simdata->simapiversion == SIMAPI_VERSION) {
        ...
        char *temp;
        asprintf(&temp,
                 "skipping sim api daemon due to version mismatch. Daemon "
                 "Version: %i. App Version: %i",
  ...
  case SIMULATOREXE_ASSETTO_CORSA_EVO:
  case SIMULATOREXE_ASSETTO_CORSA_RALLY:
    simapi_log(SIMAPI_LOGLEVEL_DEBUG,
               "Found running process for Assetto Corsa");
    if (does_sim_file_exist("/dev/shm/acpmf_physics")) {
      if (does_sim_file_exist("/dev/shm/acpmf_static")) {
        ...
        free(temp);
        simdatamap(simdata, simmap, NULL, SIMULATORAPI_ASSETTO_CORSA, false,
                   NULL);
        ...
        simapi_log(SIMAPI_LOGLEVEL_DEBUG,
                   "Could not find static shared memory file");
      }
    } else {
      simapi_log(SIMAPI_LOGLEVEL_DEBUG,
                 "Could not find physics shared memory file");
    }
    break;
  ...
  case SIMULATOREXE_AUTOMOBILISTA2:
    if (force_udp == false) {
      if (does_sim_file_exist("/dev/shm/$pcars2$")) {
        ...
        simdatamap(simdata, simmap, NULL, SIMULATORAPI_PROJECTCARS2, false,
                   NULL);
        if (error == 0) {
          simdata->simon = true;
      ...
      if (error == 0) {
        si.simulatorapi = SIMULATORAPI_PROJECTCARS2;
        si.SimUsesUDP = true;
        simdatamap(simdata, simmap, NULL, SIMULATORAPI_PROJECTCARS2, true,
                   NULL);
      }
      if (error == 0 && simdata->simstatus > 1) {
        simdata->simon = true;
...
int siminit(SimData *simdata, SimMap *simmap, SimulatorAPI simulator) {
  ...
  case SIMULATORAPI_SIMAPI_TEST:
    ...
    simmap->addr =
        mmap(NULL, sizeof(SimData), PROT_READ, MAP_SHARED, simmap->fd, 0);
  ...
  case SIMULATORAPI_PROJECTCARS2:
    ...
    simmap->pcars2.fd_telemetry =
        shm_open(PCARS2_FILE, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (simmap->pcars2.fd_telemetry == -1) {
      // slogd("could not open Assetto Corsa physics engine");
      return SIMAPI_ERROR_NODATA;
    }
    simmap->pcars2.telemetry_map_addr =
        mmap(NULL, sizeof(simmap->pcars2.pcars2_telemetry), PROT_READ,
             MAP_SHARED, simmap->pcars2.fd_telemetry, 0);
  ...
  case SIMULATORAPI_RFACTOR2:
    ...
    simmap->rf2.fd_telemetry =
        shm_open(RF2_TELEMETRY_FILE, O_RDONLY, S_IRUSR | S_IWUSR);
    if (simmap->rf2.fd_telemetry == -1) {
      // slogd("could not open RFactor2 Telemetry engine");
      return SIMAPI_ERROR_NODATA;
    }
    simmap->rf2.telemetry_map_addr =
        mmap(NULL, sizeof(simmap->rf2.rf2_telemetry), PROT_READ, MAP_SHARED,
             simmap->rf2.fd_telemetry, 0);
    if (simmap->rf2.telemetry_map_addr == MAP_FAILED) {
      // slogd("could not retrieve RFactor2 telemetry data");
      return 30;
    }
    simmap->rf2.has_telemetry = true;
    simmap->rf2.fd_scoring =
        shm_open(RF2_SCORING_FILE, O_RDONLY, S_IRUSR | S_IWUSR);
    if (simmap->rf2.fd_scoring == -1) {
      // slogd("could not open RFactor2 Telemetry engine");
      return SIMAPI_ERROR_NODATA;
    }
    simmap->rf2.scoring_map_addr =
        mmap(NULL, sizeof(simmap->rf2.rf2_scoring), PROT_READ, MAP_SHARED,
             simmap->rf2.fd_scoring, 0);
  ...
  case SIMULATORAPI_SCSTRUCKSIM2:
    ...
    simmap->scs2.fd_telemetry =
        open("/dev/shm/SCS/SCSTelemetry", O_CREAT | O_RDWR, 0777);
    // simmap->scs2.fd_telemetry = shm_open(SCS2_TELEMETRY_FILE, O_RDONLY,
    // S_IRUSR | S_IWUSR);
    if (simmap->scs2.fd_telemetry == -1) {
      return SIMAPI_ERROR_NODATA;
    }
    simmap->scs2.telemetry_map_addr =
        mmap(NULL, sizeof(simmap->scs2.scs2_telemetry), PROT_READ, MAP_SHARED,
             simmap->scs2.fd_telemetry, 0);
...
int simfree(SimData *simdata, SimMap *simmap, SimulatorAPI simulator) {
  ...
  if (simmap->pcars2.has_telemetry == true) {
    if (munmap(simmap->pcars2.telemetry_map_addr,
               sizeof(simmap->pcars2.pcars2_telemetry)) == -1) {
      return 0201100;
    }
  ...
  if (simmap->rf2.has_telemetry == true) {
    if (munmap(simmap->rf2.telemetry_map_addr,
               sizeof(simmap->rf2.rf2_telemetry)) == -1) {
      return 100;
    }
  ...
  if (simmap->rf2.has_scoring == true) {
    if (munmap(simmap->rf2.scoring_map_addr, sizeof(simmap->rf2.rf2_scoring)) ==
        -1) {
      return 100;
    }
  ...
  if (simmap->scs2.has_telemetry == true) {
    if (munmap(simmap->scs2.telemetry_map_addr,
               sizeof(simmap->scs2.scs2_telemetry)) == -1) {
      return 100;
    }
...
int opensimmap(SimMap *simmap) {
  ...
  void *addr =
      mmap(NULL, sizeof(SimData), PROT_WRITE, MAP_SHARED, simmap->fd, 0);
  if (addr == MAP_FAILED) {
    printf("mmap");
    return 30;
  }
...
int opensimcompatmap(SimCompatMap *compatmap) {
  ...
  void *addr =
      mmap(NULL, PCARS2_SIZE, PROT_WRITE, MAP_SHARED, compatmap->pcars2_fd, 0);
  if (addr == MAP_FAILED) {
    printf("mmap");
    return 30;
  }
  ...
  addr = mmap(NULL, AC_PHYSICS_SIZE, PROT_WRITE, MAP_SHARED,
              compatmap->acphysics_fd, 0);
  if (addr == MAP_FAILED) {
    printf("mmap");
    return 30;
  }
  ...
  addr = mmap(NULL, AC_GRAPHIC_SIZE, PROT_WRITE, MAP_SHARED,
              compatmap->acgraphics_fd, 0);
  if (addr == MAP_FAILED) {
    printf("mmap");
    return 30;
  }
  ...
  addr = mmap(NULL, AC_STATIC_SIZE, PROT_WRITE, MAP_SHARED,
              compatmap->acstatic_fd, 0);
  if (addr == MAP_FAILED) {
    printf("mmap");
    return 30;
  }
  ...
  addr = mmap(NULL, AC_CREWCHIEF_SIZE, PROT_WRITE, MAP_SHARED,
              compatmap->accrew_fd, 0);
  if (addr == MAP_FAILED) {
    printf("mmap");
    return 30;
  }
